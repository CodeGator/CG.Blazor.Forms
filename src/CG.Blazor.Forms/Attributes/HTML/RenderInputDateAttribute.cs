...
/// causes the form generator to render the property as an date HTML 
/// element.
/// </summary>
/// <remarks>
/// <para>
...
/// </remarks>
/// <example>
/// Here is an example of decorating a model property to render a date:
/// <code>
...
public class RenderInputDateAttribute : HtmlAttribute
{
    // *******************************************************************
    // Properties.
    // *******************************************************************
    ...
    /// <summary>
    /// This constructor creates a new instance of <see cref="RenderInputDateAttribute"/>
    /// class.
    /// </summary>
    public RenderInputDateAttribute()
    {
        // Set default values.
        Max = string.Empty;
        Min = string.Empty;
        ...
        )
    {
        ...
        try
        {
            // If we get here then we are trying to render a date HTML element
            ...
            if (path.Count < 2)
            {
                ...
                logger.LogDebug(
                    "RenderInputDateAttribute::Generate called with a shallow path!"
                    );
            ...
            if (null == model)
            {
                // Let the world know what we're doing.
                logger.LogDebug(
                    "RenderInputDateAttribute::Generate called with a null model!"
                    );
            ...
            // We only render date controls against datetimes.
            if (modelType == typeof(DateTime))
            {
                // Let the world know what we're doing.
                logger.LogDebug(
                    "Rendering property: '{PropName}' as a date element.",
                    prop.Name
                    );
                ...
                attributes["value"] = ((DateTime)prop.GetValue(propParent)).ToString("yyyy-MM-dd");
                ...
                attributes["onchange"] = RuntimeHelpers.TypeCheck<EventCallback<ChangeEventArgs>>(
                    EventCallback.Factory.Create<ChangeEventArgs>(
                        ...
                        EventCallback.Factory.CreateInferred<ChangeEventArgs>(
                            ...
                            new ChangeEventArgs()
                            {
                                Value = ((DateTime)prop.GetValue(propParent)).ToString("yyyy-MM-dd")
                            })
                        )
                    );
            ...
            else
            {
                ...
                logger.LogDebug(
                    ...
                    "because we only render date elements on properties " +
        ...
        catch (Exception ex)
        {
            ...
            throw new FormGenerationException(
                message: "Failed to render an input element! " +
